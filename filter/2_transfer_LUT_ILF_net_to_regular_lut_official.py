...
def transfer_LUT_ILF_net_to_regular_lut(opt):
    ...
    model = getattr(Model, 'LUT_ILF_Net')
    ...
    lm = load_checkpoint_state_dict(
        os.path.join(opt.loadDir, 'Model_{:06d}.pth'.format(opt.loadIter)),
        map_location='cpu',
    )
    ...
    # conv5
    input_tensor = input_tensor_ori.reshape((-1,4,1,1))
