...
from data import Provider, SRBenchmark, Benchmark
...
if __name__ == "__main__":
    ...
    model_class = {
        'standard': model.LUT_ILF_Net,
        'rfd1': model.LUT_ILF_Net_RDd1,
    }[opt.yModel]

    if opt.weight:
        model_G = model_class(nf=opt.nf, scale=opt.scale, stage1_modes=opt.stage1_modes, stage2_modes=opt.stage2_modes, stage3_modes=opt.stage3_modes, stage4_modes=opt.stage4_modes, stages=stages, weight=opt.weight, ps_error=opt.ps_error).cuda()
    else:
        model_G = model_class(nf=opt.nf, scale=opt.scale, stage1_modes=opt.stage1_modes, stage2_modes=opt.stage2_modes, stage3_modes=opt.stage3_modes, stage4_modes=opt.stage4_modes, stages=stages, ps_error=opt.ps_error).cuda()
    ...
    if opt.steplr:
        ...
        scheduler = optim.lr_scheduler.StepLR(opt_G, step_size=opt.Decaystep, gamma=opt.reduce)
    else:
    ...
    if opt.startIter > 0:
        if opt.testModel:
            checkpoint_path = opt.testModelPath
        elif opt.resumeModel:
            checkpoint_path = opt.resumeModelPath
        else:
            checkpoint_path = find_model_checkpoint(
                opt.loadDir,
                opt.startIter,
                channel='Y' if opt.yModel == 'rfd1' else None,
            )
        ...
        logger.info("Model Loading: {}".format(checkpoint_path))

        # Evaluation-only mode does not need optimizer/scheduler state.
        if not opt.validation:
            if opt.resumeModel:
                optimizer_path = opt.resumeOptimizerPath
                if not os.path.isfile(optimizer_path):
                    raise FileNotFoundError(
                        "Optimizer checkpoint not found: {}".format(optimizer_path)
                    )
            else:
                optimizer_names = []
                if opt.yModel == 'rfd1':
                    optimizer_names.append('Opt_Y_{:06d}.pth'.format(opt.startIter))
                optimizer_names.append('Opt_{:06d}.pth'.format(opt.startIter))
                optimizer_path = None
                for optimizer_name in optimizer_names:
                    candidate = os.path.join(opt.loadDir, optimizer_name)
                    if os.path.isfile(candidate):
                        optimizer_path = candidate
                        break
                if optimizer_path is None:
                    raise FileNotFoundError(
                        "No optimizer checkpoint found; tried {}".format(
                            ', '.join(optimizer_names)
                        )
                    )

            optimizer_payload = torch.load(optimizer_path, map_location='cpu')
            # Historical releases stored a complete Adam object, whereas new
            # checkpoints store only its state_dict.
            if isinstance(optimizer_payload, optim.Optimizer):
                optimizer_payload = optimizer_payload.state_dict()
            if not isinstance(optimizer_payload, dict) or not {
                    'state', 'param_groups'} <= set(optimizer_payload):
                raise TypeError(
                    "Unsupported optimizer checkpoint format: {}".format(
                        optimizer_path
                    )
                )
            opt_G.load_state_dict(optimizer_payload)
            logger.info("Optimizer Loading: {}".format(optimizer_path))

        # lr scheduler
        if opt.schedule and not opt.validation:
            ...
            if not opt.validation:
                # No historical scheduler file is bundled. Reconstruct the
                # scheduler position so the next step uses iteration N+1's LR.
                scheduler.last_epoch = opt.startIter
                if isinstance(scheduler, optim.lr_scheduler.LambdaLR):
                    resumed_lrs = [
                        base_lr * lr_lambda(opt.startIter)
                        for base_lr, lr_lambda in zip(
                            scheduler.base_lrs, scheduler.lr_lambdas
                        )
                    ]
                elif hasattr(scheduler, '_get_closed_form_lr'):
                    resumed_lrs = scheduler._get_closed_form_lr()
                else:
                    resumed_lrs = scheduler.get_lr()
                for param_group, lr in zip(opt_G.param_groups, resumed_lrs):
                    param_group['lr'] = lr
                scheduler._last_lr = resumed_lrs
                logger.info(
                    "Scheduler aligned to Iter {} | lr: {}".format(
                        opt.startIter, resumed_lrs
                    )
                )
    ...
    if opt.startIter > 0:
        if opt.gpuNum > 1:
            if opt.task == 'in-loop-filter':
                valid_steps_postprocessing(model_G.module, valid, opt, opt.startIter)
            else:
                valid_steps(model_G.module, valid, opt, opt.startIter)
        else:
            if opt.task == 'in-loop-filter':
                valid_steps_postprocessing(model_G, valid, opt, opt.startIter)
            else:
    ...
    for i in range(opt.startIter + 1, opt.totalIter + 1):
        ...
        if i % opt.saveStep == 0 and i % opt.valStep == 0 and i >= opt.valStartIter:
            if opt.gpuNum > 1:
                if opt.task == 'in-loop-filter':
                    valid_steps_postprocessing(model_G.module, valid, opt, i)
                else:
                    valid_steps(model_G.module, valid, opt, i)
            else:
                if opt.task == 'in-loop-filter':
                    valid_steps_postprocessing(model_G, valid, opt, i)
                else:
